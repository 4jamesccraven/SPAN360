...
class Sentence:
    ...
    def from_str(string: str) -> Sentence:
        ...
        try:
            ...
            raise ValueError(
                f'Mal-formed identifier, could not convert to int: {string}'
            )
...
def parse_data() -> Optional[list[SentencePairing]]:
    ...
    try:
        with open('./corpus.dat', 'r', encoding='utf8') as f:
            ...
            corpus = [l for line in f if (l := line.strip()) and not l.startswith('#')]

        records = map(Sentence.from_str, corpus)

    except FileNotFoundError:
        print(
            'Corpus not found. Ensure that you are running this'
    ...
    if not 'NO_TQDM' in environ:
        try:
            ...
            print('Unable to load tqdm. Proceeding without.', file=stderr)
